...
import {
  DefaultDAOInterface,
  Query,
} from '../app/interfaces/defaultDAOInterface';
...
export class DefaultDAO extends DefaultDAOInterface {
  ...
  updateByReference(
    ...
    return this.firestore
      .doc<any>(target + '/' + reference)
      .ref.update(Object.assign({}, object));
  }
  ...
  listAllByAttribute(
    ...
    if (limit != undefined && limit >= 1) {
      return this.firestore
        .collection(target)
        .ref.where(query.attribute, query.operator, value)
        .limit(limit)
        .get();
    } else {
      return this.firestore
  ...
  listAllByAttributes(
    ...
    let collection = this.firestore
      ...
      .ref.where(
        queries[0].attribute,
        queries[0].operator,
        queries[0].value.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      );
    for (let query of queries) {
      collection = collection.where(
        query.attribute,
        query.operator,
        query.value.normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      );
    }
